...
"""
This file is the **only explicit configuration entry** of DRST:
- All default parameters of S3/MinIO, Kafka, offline/online/retraining/monitoring are defined here
- Other code should import this file to read configuration, instead of relying on external export
- Only the wall time duration is allowed to be overridden at runtime by environment variables or set_* (see MAX_WALL_SECS interface at the end)
"""

# ===== MinIO / S3 (explicit configuration) =====
MINIO_ACCESS_MODE = "cluster"  # cluster / local (only for internal semantics)
MINIO_SCHEME      = "http"     # http / https
MINIO_ENDPOINT    = "minio-service.kubeflow.svc.cluster.local:9000"  # without scheme
BUCKET            = "onvm-demo2"

# Directories (S3 prefixes)
MODEL_DIR  = "models"
RESULT_DIR = "results"
DATA_DIR   = "datasets"

# ===== Kafka (explicit configuration) =====
KAFKA_SERVERS = "kafka.default.svc.cluster.local:9092"
KAFKA_TOPIC   = "latencyTopic"

# ===== Features / Training (offline) =====
FEATURE_SRC_KEY = f"{DATA_DIR}/combined.csv"
EXCLUDE_COLS    = ["Unnamed: 0", "input_rate", "latency"]
TARGET_COL      = "output_rate"
OFFLINE_TOPK    = 10
ACC_THR         = 0.25  # Cumulative hit threshold (used for cumulative accuracy in inference)
...
INFER_HIT_THR       = 0.15  # In-batch “hit” threshold (for logging/metrics)

# ===== Producer (rate and segmentation) =====
# Note: If runtime pipeline parameters are not provided, execution will follow this file’s configuration; no need to rely on environment variables.
PRODUCER_TPS        = 10.0   # If >0, approximate records per second ≈ TPS
PRODUCE_INTERVAL_MS = 100    # If TPS<=0, use fixed interval (milliseconds)
...
# In-process temporary override (testing/hot-swap); None to cancel override
...
# ===== Pause/Resume Control During Retraining (monitor<->infer) =====
MONITOR_SIGNAL_INFER_PAUSE = False  # Whether to send a “pause” flag to infer when monitor triggers retraining
INFER_RESPECT_PAUSE_FLAG   = True   # Whether infer responds to the pause flag
PAUSE_INFER_KEY            = f"{RESULT_DIR}/pause_infer.flag"
MONITOR_WAIT_RETRAIN       = False  # Retain old name to prevent external reference errors (logic already built-in)
