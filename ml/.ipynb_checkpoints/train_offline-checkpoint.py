...
bridge = load_csv(pathlib.Path(DATA_DIR, "total",
...
joblib.dump(scaler, pathlib.Path(MODEL_DIR, "scaler.pkl"))
joblib.dump(pca,    pathlib.Path(MODEL_DIR, "pca.pkl"))
torch.save(model.state_dict(), pathlib.Path(MODEL_DIR, "mlp.pt"))
np.save (pathlib.Path(MODEL_DIR, "base_acc.npy"), base_acc)   # ★ 新增
